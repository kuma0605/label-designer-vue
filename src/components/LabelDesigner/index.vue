...
// Watch modelValue from parent to load template inside storeList
watch(
  () => props.modelValue,
  (newVal) => {
    if (newVal) {
...
// Watch storeList inside local store to automatically sync back via v-model
watch(
...
const handleCancelCurrent = (e) => {
  // If the user clicks inside properties controls or left materials toolbox, do not cancel selection
  ...
  if (leftMenu && leftMenu.contains(e.target)) {
    return;
  }
  ...
  // Deselect active component and clear multi-selection
  actions.setActive('');
...
<template>
  <div class="label-designer-main-container">
    <HeaderNav @save="handleSave" @print="handlePrint" />
