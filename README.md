...
| 模板管理库 | 标签设计器（变量字段） |
| :---: | :---: |
| ![模板管理库](screenshots/1.png) | ![标签设计器 - 动态变量](screenshots/2.png) |
| 浏览、编辑、导入内置模板 | 拖拽变量占位符，绑定资产字段 |

| 标签设计器（绘图物料） | 标签打印预览 |
| :---: | :---: |
| ![标签设计器 - 基础物料](screenshots/3.png) | ![标签打印预览弹窗](screenshots/4.png) |
| 文本、条码、二维码、表格等组件 | 预览填数 + QZ Tray / 浏览器打印 |
...
2. **动态变量绑定**：支持 `$_{asset_num}` 等占位符，模板与真实资产数据分离；预览/打印时注入 `variables` 替换。详见 [`docs/05_template-variables-and-device-print.md`](docs/05_template-variables-and-device-print.md)。
